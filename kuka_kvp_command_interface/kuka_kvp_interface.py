...
def format_joint_states(joint_states: list):
    ...
    if joint_states:
        ...
        formatted_joint_state = "{E6AXIS: A1 " + f"{a1}," + " A2 " + f"{a2}," + " A3 "+f"{a3}," + " A4 " + \
            f"{a4}," + " A5 " + f"{a5}," + " A6 " + f"{a6}," + \
...
def format_linear_position(position: list, rotation: list):
    ...
    if position:
        ...
        formated_linear_position = "{POS: X " + f"{X}," + " Y " + f"{Y}," + \
            " Z "+f"{Z}," + " A " + f"{A}," + \
            " B " + f"{B}," + " C " + f"{C}" + "}"
...
def format_actual_robot_position(robot_position) -> str:
    ...
    clean_reduced_position = [
        round(float(pos.split(" ")[-1].strip("}")), 3) for pos in reduced_position]
    print(f'clean_reduced_position: {clean_reduced_position}')

    formatted_position = "{POS: X " + f"{clean_reduced_position[0]}," + " Y " + f"{clean_reduced_position[1]}," + " Z "+f"{clean_reduced_position[2]}," + \
        " A " + f"{clean_reduced_position[3]}," + " B " + \
        f"{clean_reduced_position[4]}," + " C " + \
        f"{clean_reduced_position[5]}" + "}"
...
def calculate_error(current_position, next_position):
    ...
    cur_pos = [float(i.split(" ")[-1].strip("}"))
               for i in current_position.split(", ")[:3]]
    next_pos = [float(i.split(" ")[-1].strip("}"))
                for i in next_position.split(", ")[:3]]
...
# Orchestrator function; checks the queue and sends data to the robot
def feedback_loop():
    ...
    # TODO: Re-enable logging once tested
    print(
        f'Sending to robot (count) - (next_position) - (next_linear_velocity) - (torch_action) : ({coordinate_queue.counter()}) - ({next_position}) - ({next_linear_velocity}) - ({next_torch_action})')
    ...
    if len(current_position.encode('utf-8')) < 100:
        # TODO: Re-enable logging once tested
        print(
            f'Invalid Robot position (size) - (reading current_position):{len(current_position.encode("utf-8"))} - {current_position}')
        return
    ...
    if calculate_error(current_position, next_position):
        ...
        # TODO: Re-enable logging once tested
        print(
            f'Not done moving >>> count - current_position: {coordinate_queue.counter()} - {current_position}')
        return
...
def linear_position_callback_fn(subscribedData):
    ...
    position_velocity_info = format_linear_position(
        linear_position_value, linear_position_rotation)
    ...
    position_velocity_object = [
        formated_linear_position, linear_velocity, formatted_torch_action]

    coordinate_queue.enqueue(position_velocity_object)

    # TODO: Re-enable logging once tested
    print(
        f'Adding to coordinate_queue (position_velocity_object): {position_velocity_object}')
    print(f'Buffer Size: {len(coordinate_queue)}  coordinates')
...
class KVPInterfaceNode(Node):
    def __init__(self):
        ...
        self.torch_command_publisher = self.create_publisher(
            String, '/torch', 10)
